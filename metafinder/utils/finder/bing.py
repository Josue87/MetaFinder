...
def search(target, total):
    ...
    with contextlib.suppress(Exception):
        ...
        while (count < iter_count) and (len(documents) < total):
            ...
            new_url = f"{url}&first={this_count}&FORM=PERE"
            req = Request(new_url,
            ...
            for link in all_links:
                ...
                if href and target in href and \
                        (href.endswith("pdf") or href.endswith("doc") or href.endswith("docx") or href.endswith(
                            "ppt") or href.endswith("pptx") or href.endswith("xls") or href.endswith("xlsx")) \
